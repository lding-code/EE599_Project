...
export class ProbSolveComponent implements OnInit {
  ...
  // send code to backend for evaluation when submit button is clicked
  onSubmit(){
  ...
  // initialize code editor when the component is initialized
  OnInit(editor) {
    let line = editor.getPosition();
    console.log(line);
  }

  // initialize data sevice when component is initialized
  ngOnInit() {
  ...
  // unsubscribe data service when component is deleted.
  ngOnDestroy() {
    this.probInSub.unsubscribe();
  }
}
